...
export default function AncientMassiveGalaxiesPage() {
  ...
  const data = discoveriesData.discoveries[0];

  useEffect(() => {
    if (hasUserConsented && audioRefDefault.current) {
      audioRefDefault.current.volume = 0.0; // Default audio volume
  ...
  const handleTogglePlayPauseAll = () => {
    ...
    if (isAllPlaying) {
      ...
      if (audioRefDefault.current) {
        audioRefDefault.current.volume = 0.0; // Pause the background music
      }
  ...
  return (
    <Box height="100vh" width="100vw" position="relative" bg="black">
      ...
      <audio
        ref={audioRefDefault}
        src={`${process.env.PUBLIC_URL}/assets/music/ancient_massive_galaxies/bass.wav`}
        loop
      />
      ...
      {data.characteristics.slice(0, 3).map((char, index) => (
        ...
        >
          ...
          <audio
            ...
            src={
              index === 0
                ? `${process.env.PUBLIC_URL}/assets/music/ancient_massive_galaxies/percussion.wav`
                : index === 1
                ? `${process.env.PUBLIC_URL}/assets/music/ancient_massive_galaxies/percussion2.wav`
                : `${process.env.PUBLIC_URL}/assets/music/ancient_massive_galaxies/others.wav`
            }
            loop
          />
        </Box>
